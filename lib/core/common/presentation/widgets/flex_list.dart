...
class RenderFlexList extends RenderBox
  ...
  void performLayout() {
    ...
    var first = true;
    while (child != null) {
      final hSpacing = first ? 0 : horizontalSpacing;
      final size = child.getDryLayout(childConstraint);
      final parentData = child.parentData! as _FlexListParentData
        .._initSize = size;
...
class _RowMetrics {
  // where width is the some of all widths and height is the heighest element

  _RowMetrics(this.childNumber, this.contentRawSize);
  final int childNumber;
  final Size contentRawSize;
}
